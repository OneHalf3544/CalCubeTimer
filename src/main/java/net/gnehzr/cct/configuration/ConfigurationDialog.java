...
public class ConfigurationDialog extends JDialog {
            ...
            if (source instanceof JColorComponent) {
                ...
                if (source == timerFontChooser || source == scrambleFontChooser) {
                    ...
                    Color selected = JColorChooser.showDialog(
                            ...
                            StringAccessor.getString("ConfigurationDialog.choosecolor"),
                            label.getBackground());
                    if (selected != null)
    ...
    private void createGUI() {
        ...
        tabbedPane.addTab(StringAccessor.getString("ConfigurationDialog.options"), makeStandardOptionsPanel1());
        tabbedPane.addTab(StringAccessor.getString("ConfigurationDialog.moreoptions"), makeStandardOptionsPanel2());
        tabbedPane.addTab(StringAccessor.getString("ConfigurationDialog.scramcustomizations"), makeScrambleTypeOptionsPanel());
        tabbedPane.addTab(StringAccessor.getString("ConfigurationDialog.stackmatsettings"), makeStackmatOptionsPanel());
        tabbedPane.addTab(StringAccessor.getString("ConfigurationDialog.statistics"), makeStatisticsPanels());
        tabbedPane.addTab(StringAccessor.getString("ConfigurationDialog.colors"), makePuzzleColorsPanel());

        JButton applyButton = new JButton(StringAccessor.getString("ConfigurationDialog.apply"));
        applyButton.setMnemonic(KeyEvent.VK_A);
        applyButton.addActionListener(e -> applyAndSave());

        JButton saveButton = new JButton(StringAccessor.getString("ConfigurationDialog.save"));
        ...
        JButton cancelButton = new JButton(StringAccessor.getString("ConfigurationDialog.cancel"));
        cancelButton.setMnemonic(KeyEvent.VK_C);
        cancelButton.addActionListener(e -> setVisible(false));

        JButton resetAllButton = new JButton(StringAccessor.getString("ConfigurationDialog.resetall"));
    ...
    private JButton refreshDesktopsButton;
    JComboBox voices;
    ...
    private JPanel makeStandardOptionsPanel1() {
        ...
        desktopPanel = new JPanel(); //this gets populated in refreshDesktopsButton()
        refreshDesktopsButton = new JButton(StringAccessor.getString("ConfigurationDialog.refresh"));
        refreshDesktopsButton.addActionListener(e -> refreshDesktops());
    ...
    private JPanel makeStandardOptionsPanel2() {
        ...
        splitsKeySelector.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if (!KeyboardHandler.ignoreKey(e, false, false, 0, 0)) {
        ...
        metronomeCheckbox = new JCheckBox(StringAccessor.getString("ConfigurationDialog.metronome"));
        metronomeCheckbox.addActionListener(e -> metronomeDelay.setEnabled(metronomeCheckbox.isSelected()));
        metronomeDelay = new TickerSlider(metronome);
        panel.add(sideBySide(null, metronomeCheckbox, new JLabel(StringAccessor.getString("ConfigurationDialog.delay")), metronomeDelay));

        stackmatEmulation = new JCheckBox(StringAccessor.getString("ConfigurationDialog.emulatestackmat"));
        stackmatEmulation.addActionListener(e -> {
        ...
        isBackground.addActionListener(e -> {
        ...
        SyncGUIListener al = defaults -> {
            ...
            minSplitTime.setEnabled(splits.isSelected());
            metronomeCheckbox.setSelected(configuration.getBoolean(VariableKey.METRONOME_ENABLED, defaults));
            metronomeDelay.setEnabled(metronomeCheckbox.isSelected());
    ...
    private JPanel makeStackmatOptionsPanel() {
        ...
        JButton stackmatRefresh = new JButton(StringAccessor.getString("ConfigurationDialog.refreshmixers"));
        stackmatRefresh.addActionListener(e -> {
            items = getMixers();
            int selected = stackmat.getSelectedMixerIndex();
        ...
        options.add(sideBySide(
                ...
                sideBySide(
                        null,
                        new JLabel(StringAccessor.getString("ConfigurationDialog.samplingrate")),
                        stackmatSamplingRate),
                reset));
    ...
    private JEditorPane getStatsLegend() {
        ...
        pane.setText("<html><font face='" + pane.getFont().getFontName() + "'><a href=''>" +
                StringAccessor.getString("ConfigurationDialog.seedynamicstrings") + "</a></font>");
    ...
    void refreshDesktops() {
        ...
        for (int ch = 0; ch < screenDevices.length; ch++) {
            JRadioButton desktopRadioButton = createDesktopRadioButton(screenDevices[ch], ch);
            buttonGroup.add(desktopRadioButton);
            desktopPanel.add(desktopRadioButton);
        }

        desktopPanel.add(refreshDesktopsButton);
        if (focused != null) {
    ...
    @NotNull
    private JRadioButton createDesktopRadioButton(GraphicsDevice screenDevice, int ch) {
        DisplayMode screenSize = screenDevice.getDisplayMode();
        JRadioButton desktopRadioButton = new JRadioButton(displayAsString(ch, screenSize));

        if (ch == configuration.getInt(VariableKey.FULLSCREEN_DESKTOP)) {
            desktopRadioButton.setSelected(true);
        }

        desktopRadioButton.setActionCommand(Integer.toString(ch));
        desktopRadioButton.addActionListener(e -> configuration.setLong(VariableKey.FULLSCREEN_DESKTOP, ch));
        return desktopRadioButton;
    }

    @NotNull
    private String displayAsString(int ch, DisplayMode screenSize) {
        return (ch + 1) + ":"
    ...
    private void applyAndSave() {
        ...
        configuration.setBoolean(VariableKey.METRONOME_ENABLED, metronomeCheckbox.isSelected());
